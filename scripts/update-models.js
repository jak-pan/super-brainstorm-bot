  ...
  try {
    ...
    const chatModels = models
      .filter(m => {
        ...
        return id.startsWith('gpt-') || id.startsWith('o') || id.includes('chat');
      })
      .map(m => ({
        id: m.id,
        name: m.id,
    ...
    console.log('⚠️  No models from API, using known current models as fallback');
    return [
...
function getOpenAIPricing(modelId) {
  ...
  if (id.includes('gpt-5') && id.includes('mini')) return pricingMap['gpt-5-mini'];
  if (id.includes('gpt-5') && id.includes('nano')) return pricingMap['gpt-5-nano'];
  if (id.includes('gpt-5')) return pricingMap['gpt-5'];
  if (id.includes('gpt-4o') && id.includes('mini')) return pricingMap['gpt-4o-mini'];
  if (id.includes('gpt-4o')) return pricingMap['gpt-4o'];
  if (id.includes('gpt-4-turbo')) return pricingMap['gpt-4-turbo'];
  if (id.includes('gpt-4') && !id.includes('turbo') && !id.includes('o')) return pricingMap['gpt-4'];
...
async function fetchAnthropicModels(apiKey) {
  try {
    ...
    console.log(`✅ Using known Anthropic models: ${knownModels.length} models`);
...
async function fetchGrokModels(apiKey) {
  try {
    if (apiKey) {
      ...
      console.log('ℹ️  GROK_API_KEY not set, using known Grok models as fallback');
    }
...
function getContextWindow(provider, modelId) {
  ...
  if (provider === 'openai') {
    ...
    if (id.includes('gpt-5') || id.includes('gpt-4o') || id.includes('gpt-4-turbo') || id.includes('o3')) {
      return 128000;
    }
...
function isSmartestModel(provider, modelId, allModelIds) {
  ...
  if (provider === 'openai') {
    ...
    if (id.includes('gpt-5') && !id.includes('mini') && !id.includes('nano')) return true;
    if (id.includes('gpt-4o') && !id.includes('mini')) return true;
    return false;
  }
  if (provider === 'anthropic') {
    // Claude Opus 4.1 is smartest, then Sonnet 4.5, then 3.5 Sonnet
    if (id.includes('opus') && (id.includes('4') || id.includes('4.1'))) return true;
    if (id.includes('sonnet') && (id.includes('4') || id.includes('4.5'))) return true;
...
async function updateModels() {
  ...
  try {
    ...
    if (openaiModels && openaiModels.length > 0) {
      ...
      newModels.openai.smartestModel = smartestModelId || newModels.openai.models[0]?.id;
    ...
    if (anthropicModels && anthropicModels.length > 0) {
      ...
      newModels.anthropic.smartestModel = smartestModelId || newModels.anthropic.models[0]?.id;
    ...
    if (grokModels && grokModels.length > 0) {
      ...
      newModels.grok.smartestModel = smartestModelId || newModels.grok.models[0]?.id;
      newModels.grok.defaultModel = newModels.grok.models[0]?.id;
    }

    // Write updated models.json
    writeFileSync(MODELS_FILE, JSON.stringify(newModels, null, 2) + '\n', 'utf-8');
    ...
    console.log('   To update pricing, check official pricing pages and update');
    console.log('   getOpenAIPricing(), getAnthropicPricing(), or getGrokPricing() functions:');
