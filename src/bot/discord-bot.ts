...
export class DiscordBot {
  ...
  private async handleCommand(message: Message): Promise<void> {
    ...
    switch (command.toLowerCase()) {
      case "start":
      case "approve":
        await this.handleStartCommandMessage(message);
        break;
      case "continue":
        await this.handleContinueCommandMessage(message);
        break;
      case "stop":
  ...
  /**
   * Handle !start text command (legacy)
   */
  private async handleStartCommandMessage(message: Message): Promise<void> {
...
Cost: $${(conversation.costTracking?.totalCost || 0).toFixed(2)}
${
  ...
  private async handleContinueCommandMessage(message: Message): Promise<void> {
    const conversationId = this.activeConversations.get(message.channel.id);
    if (!conversationId) {
      await message.reply("No active conversation found.");
      return;
